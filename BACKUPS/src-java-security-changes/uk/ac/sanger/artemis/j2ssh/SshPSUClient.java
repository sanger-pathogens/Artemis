...
public class SshPSUClient extends Thread
{
  ...
  public void run()
  {
    ...
    try
    {
    	  // We no longer need this call in sshtools Maverick release..
  ...
  */
             ...
             throws SshException, IOException
  {
    ...
    catch(SshException | SftpStatusException sshe)
    {
      sftp = getSftpClient();
      ...
      {
    	  	throw new SshException(sse.getMessage(), sse.getStatus(), sse);
      }
    }
                    ...
                    throws IOException
  {
    ...
    for(int i=0; i<seqfile.size();i++)
    {
      ...
      if(i == 0)
      {
        ...
        {
          // Directory already created FAILURE status
        	  // will be thrown by mkdir method.
        }
      ...
      catch(TransferCancelledException | SshException | SftpStatusException sshe)
      {
      ...
      catch(ChannelOpenException | SshException exp)
      {
  ...
  */
             ...
             throws IOException
  {
    ...
    try
    {
      //  TODO KEV - there was an ftp client open check here - now just instantiates a new one
      sftp = new SftpClient(ssh);
    }
    catch(Exception e1)
    {
    	  logger4j.debug("getSftpClient() -- 1");
      if(System.getProperty("debug") != null)
      {
        e1.printStackTrace();
      }
  ...
  */
  class StdoutStdErrHandler extends Thread
  {
    ...
    protected StdoutStdErrHandler(final SshSession session, 
